...
class AssetEncryptor(private val secretKey: SecretKey) {
    ...
    fun encrypt(data: ByteArray, assetPath: String): ByteArray {
        ...
        return try {
            ...
            // 优先使用 C 优化引擎 (恒定时间 AES, 标准 PBKDF2)
            val encrypted = if (NativeCryptoOptimized.isAvailable()) {
                val keyBytes = secretKey.encoded
                NativeCryptoOptimized.encryptWithKey(data, keyBytes, aad)
            } else {
                null
            } ?: run {
                // 回退到 Java 实现
                AesCryptoEngine.encryptWithKey(
                    plainData = data,
                    secretKey = secretKey,
                    associatedData = aad
                )
