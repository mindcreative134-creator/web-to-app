...
class GeckoViewEngine(
    ...
    private fun setupNavigationDelegate(session: GeckoSession, callback: BrowserEngineCallback) {
        session.navigationDelegate = object : GeckoSession.NavigationDelegate {
            ...
            override fun onLoadRequest(
                ...
                // Google OAuth interception: Google blocks sign-in from embedded WebViews/GeckoView
                // (Error 403: disallowed_useragent). Redirect to system browser instead.
                if (isGoogleOAuthUrl(uri)) {
                    AppLogger.d("GeckoViewEngine", "Google OAuth detected — opening in system browser: $uri")
    ...
    private val oauthExternalBrowserHosts = setOf(
        "accounts.google.com",
    ...
    private fun isGoogleOAuthUrl(url: String): Boolean {
        ...
        // Direct host match
        if (oauthExternalBrowserHosts.any { oauthHost ->
            host == oauthHost || host.endsWith(".$oauthHost")
        }) {
            return true
        }
        
        // Google OAuth path pattern detection
