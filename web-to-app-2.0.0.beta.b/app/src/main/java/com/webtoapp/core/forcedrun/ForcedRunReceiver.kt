...
class ForcedRunReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        AppLogger.e(TAG, "收到广播: action=$action, appId=$appId")
        
        when (action) {
            ForcedRunManager.ACTION_FORCED_RUN_START -> {
                // Start强制运行
                AppLogger.e(TAG, "启动强制运行: appId=$appId")
                // 这里可以启动应用的 Activity
            }
            
            ForcedRunManager.ACTION_FORCED_RUN_END -> {
                // End强制运行
                AppLogger.e(TAG, "结束强制运行: appId=$appId")
            ...
            ForcedRunManager.ACTION_FORCED_RUN_CHECK -> {
                // 定时检查
                AppLogger.e(TAG, "定时检查强制运行状态")
            }
            
            Intent.ACTION_BOOT_COMPLETED -> {
                // 开机启动，检查是否需要恢复强制运行状态
                AppLogger.e(TAG, "开机启动，检查强制运行状态")
            }
        }
    }
}
