...
class MainViewModel(
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                val webApp = _currentApp.value?.copy(
                    name = state.name,
                    url = normalizeUrl(state.url, state.appType),
                    iconPath = iconPath,
                    ...
                    WebApp(
                        name = state.name,
                        url = normalizeUrl(state.url, AppType.WEB),
                        iconPath = iconPath,
    ...
    private fun validateInput(state: EditState): Boolean {
        return when {
            ...
            state.appType == AppType.WEB && isInsecureRemoteHttpUrl(state.url) -> {
                _uiState.value = UiState.Error(Strings.insecureHttpBlocked)
                false
            }
    ...
    private fun normalizeUrl(url: String, appType: AppType): String {
        ...
        return upgradeRemoteHttpToHttps(withScheme)
    }
