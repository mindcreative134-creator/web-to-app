...
static int probe_flashlight_path(void) {
    ...
    for (int i = 0; i < flashlight_path_count; i++) {
        if (path_exists(flashlight_brightness_paths[i])) {
            // 尝试读取 max_brightness
            char max_path[256];
            // 从 brightness 路径推导 max_brightness 路径
            ...
            LOGI("Flashlight sysfs found: %s (max=%s)", 
                 flashlight_brightness_paths[i], cached_flash_max_brightness);
            break;
        }
    }
    
    if (cached_flash_path_idx < 0) {
        LOGW("No flashlight sysfs path found, falling back to Java API");
    }
...
static int native_set_flashlight(int on) {
    ...
    return sysfs_write(flashlight_brightness_paths[idx], value);
}
...
static void *strobe_thread_func(void *arg) {
    ...
    const char *path = flashlight_brightness_paths[idx];
    int on = 0;
...
static int probe_brightness(void) {
    ...
    if (cached_brightness_idx >= 0) {
        ...
        LOGI("Brightness sysfs: %s (max=%d)", brightness_paths[cached_brightness_idx], cached_max_brightness);
    }
...
static int native_set_brightness(int level) {
    ...
    return sysfs_write(brightness_paths[idx], buf);
}
...
static void probe_all_capabilities(void) {
    ...
    // 闪光灯
    hw_caps.flashlight_available = (probe_flashlight_path() >= 0 && 
                                     is_writable(flashlight_brightness_paths[cached_flash_path_idx]));
    ...
    hw_caps.brightness_available = (b_idx >= 0 && is_writable(brightness_paths[b_idx]));
