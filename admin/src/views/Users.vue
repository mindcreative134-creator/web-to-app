  ...
  <div class="slide-up">
    <div class="page-header">
      <div>
        ...
        <p class="text-secondary text-sm">Manage all users and subscriptions on the platform</p>
      </div>
      <span class="badge badge-secondary">{{ total }} users</span>
    </div>
    ...
    <div class="filter-row">
      <div class="search-wrap">
        ...
        <input v-model="search" class="input search-input" placeholder="Search email or username" @input="debouncedLoad" />
      </div>
    ...
    <div class="card mt-16">
      ...
      <div v-else class="table-wrap">
        <table>
          <thead><tr>
            <th>User</th><th>Password</th><th>Online Duration</th><th>Plan</th><th>Expiry</th><th>Status</th><th style="width:70px"></th>
          </tr></thead>
          <tbody>
            <tr v-for="u in users" :key="u.id" class="user-row">
              ...
              <td>
                <div class="password-cell" v-if="u.password && u.password !== '—'">
                  ...
                  <button class="btn-icon-tiny" @click="togglePw(u)" :title="u._showPw ? 'Hide' : 'Show'">
                ...
                <span v-else class="text-xs text-muted" title="Password will be recorded after user login">Waiting for login</span>
              </td>
              ...
              <td class="text-sm">
                ...
                <span v-else-if="u.pro_plan === 'lifetime'" class="text-success">Permanent</span>
                <span v-else class="text-muted">—</span>
              </td>
              <td><span :class="['badge', u.is_active ? 'badge-success' : 'badge-danger']">{{ u.is_active ? 'Active' : 'Banned' }}</span></td>
      ...
      <div class="modal modal-lg">
        ...
        <div class="detail-grid">
          <div class="detail-section">
            ...
            <div class="info-list">
              <div class="info-item"><span>Email</span><span>{{ detailUser.user.email }}</span></div>
              ...
              <div class="info-item">
                ...
                <span v-else class="text-xs text-muted">Waiting for login</span>
              </div>
              <div class="info-item"><span>Registered</span><span>{{ fmtDate(detailUser.user.created_at) }}</span></div>
              <div class="info-item"><span>Last Login</span><span>{{ fmtDate(detailUser.user.last_login_at) || '—' }}</span></div>
              <div class="info-item"><span>Login Count</span><span>{{ detailUser.user.login_count }}</span></div>
              <div class="info-item">
                <span>Online Duration</span>
                <span class="online-duration">{{ fmtDuration(detailUser.user.total_online_seconds) }}</span>
              </div>
              <div class="info-item"><span>Apps Created</span><span>{{ detailUser.user.apps_created }}</span></div>
              <div class="info-item"><span>APKs Built</span><span>{{ detailUser.user.apks_built }}</span></div>
            </div>
          </div>
          <div class="detail-section">
            <h4 class="section-label">Subscription Management</h4>
            <div class="form-group">
              <label class="form-label">Plan</label>
              <select v-model="editForm.pro_plan" class="select">
                <option value="free">Free</option>
                <option value="pro_monthly">Pro Monthly $3</option>
                <option value="pro_yearly">Pro Yearly $28.80</option>
                <option value="pro_lifetime">Pro Lifetime $99</option>
                <option value="ultra_monthly">Ultra Monthly $9</option>
                <option value="ultra_yearly">Ultra Yearly $86.40</option>
                <option value="ultra_lifetime">Ultra Lifetime $199</option>
            ...
            <div class="flex gap-16 mt-8">
              <label class="toggle-label"><input type="checkbox" v-model="editForm.is_active" class="toggle" /> Account Normal</label>
            </div>
            <button class="btn btn-primary btn-sm mt-16" @click="saveUser" :disabled="saving" style="width:100%">Save Changes</button>
          </div>
        </div>
      </div>
    </div>
...
async function saveUser() {
  ...
  try {
    ...
    showToast('Saved'); loadUsers(); openDetail(detailUser.value.user.id)
...
function planName(p) { return { free:'Free', pro_monthly:'Pro Monthly', pro_yearly:'Pro Yearly', pro_lifetime:'Pro Lifetime', ultra_monthly:'Ultra Monthly', ultra_yearly:'Ultra Yearly', ultra_lifetime:'Ultra Lifetime', lifetime:'Lifetime (Old)' }[p] || p }
...
const membershipLabel = computed(() => {
  ...
  if (p?.startsWith('ultra')) return 'Ultra Member'
  if (p?.startsWith('pro') || p === 'lifetime') return 'Pro Member'
  return 'Member'
})
// Automatically check Pro member flag when non-free plan is selected
